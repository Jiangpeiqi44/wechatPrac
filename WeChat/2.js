	...
	//console.log(req.rawBody);
		xmlParser.parseString(req.rawBody,(err,result)=>{//解构xml
			if(!err){
				...
				result.Content=result.Content.replace(/你/g,"我");
				result.Content=result.Content.replace(/吗/g,"啊!");
				result.Content=result.Content.replace(/哪/g,"这儿");
				result.Content=result.Content.replace(/不/g,'');
				result.Content=result.Content.replace(/谁/g,"奇哥");
				result.Content+="\n来自奇哥的小尾巴";//增加小尾巴
